...
function renderMangaShareButtons(manga) {
    ...
    shareSection.innerHTML = `
        ...
        <div class="share-buttons">
            <button class="share-btn facebook" title="Bagikan ke Facebook (caption otomatis disalin)" id="manga-fb-btn">
            ...
            <a href="https://api.whatsapp.com/send?text=${encodeURIComponent(shareText)}" class="share-btn whatsapp" title="Bagikan ke WhatsApp" target="_blank">
                <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
            ...
            <button class="share-btn copy-link" title="Salin Template" id="manga-share-copy-btn">
    ...
    // Facebook: salin caption dulu, lalu buka Facebook share
    const fbBtn = shareSection.querySelector("#manga-fb-btn");
    if (fbBtn) {
        fbBtn.onclick = (e) => {
            e.preventDefault();
            copyToClipboard(shareText).then(() => {
                showToast("✅ Caption disalin! Paste di kotak teks Facebook setelah terbuka.", "info");
            }).catch(() => {});
            setTimeout(() => {
    ...
    if (copyBtn) {
        copyBtn.onclick = (e) => {
            ...
            copyToClipboard(shareText)
                .then(() => showToast("✅ Template berhasil disalin!", "success"))
                .catch(() => showToast("Gagal menyalin, coba lagi", "error"));
        };
    }
}
...
function renderReaderShareButtons(mangaId, chapterNumber) {
    ...
    readerShareContainer.innerHTML = `
        ...
        <div class="reader-share-row">
            <button class="reader-share-btn facebook" id="reader-fb-btn" title="Facebook (caption otomatis disalin)">
            ...
            <a href="https://api.whatsapp.com/send?text=${encodeURIComponent(chapShareText)}" class="reader-share-btn whatsapp" target="_blank" title="WhatsApp">
            ...
            <button class="reader-share-btn copy-link" id="reader-share-copy-btn" title="Salin Template">
    ...
    // Facebook: salin caption dulu, lalu buka Facebook
    const fbBtn = readerShareContainer.querySelector("#reader-fb-btn");
    if (fbBtn) {
        fbBtn.onclick = (e) => {
            e.preventDefault();
            copyToClipboard(chapShareText).then(() => {
                showToast("✅ Caption disalin! Paste di kotak teks Facebook setelah terbuka.", "info");
            }).catch(() => {});
            setTimeout(() => {
                window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(chapShareUrl)}`, '_blank');
            }, 300);
        };
    }
